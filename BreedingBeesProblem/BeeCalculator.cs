...
namespace BreedingBeesProblem
{
    ...
    {
        public int? GetAnswer(int? source, int? destination)
        {
            KeyValuePair<int?, int?> Source = Calc(source);
            KeyValuePair<int?, int?> Destination = Calc(destination);
            ...
            return (int?)((Math.Sign(_x) == Math.Sign(_y)) ? Math.Max(Math.Abs(_x), Math.Abs(_y)) : Math.Abs(_x + _y));
        }

        private KeyValuePair<int?, int?> Calc(int? a)
        {
            //arbitrarily label 1 as (0,0)
            if (a == 1)
            {
            ...
            // looping through max numbers of levels until the max number of the layer in greater than the entered number will tell us what level the entered number is currently on
            while (3 * n * (n - 1) + 1 < a)
                n++;
            // we subtract one from the current layer to "go back" one level
            //// then we calculate the max number of that previous level
            //// then subtract the max number of that level from the entered number 
            // the result is the the position of the entered number on it's layer
            ...
            n--;
            a -= 3 * n * (n - 1) + 1;
            // then we segment the hexagon into 6 sections (because hexagon)
            ...
            // x and y was calculated by determining their relationship to n and a
            if (a > 5 * n)
            {
                x = n;
                y = -6 * n + a;
